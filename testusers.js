 ...
 * @apiDescription Фактическое измененине пароля
 * @apiVersion 1.0.0
 ...
 * @apiParam {String} hashPassword Пароль пользователя, полученыый с помощью функции хеширования
 * 
 ...
 */
 ...
 router.post("/password/change", async (req,res) => {
   const validationResult = validate(req.body, {
     ...
     hasPassword: hashPasswordConstraints
   });
           ...
           if(user.email === req.body.email) {
             Promise.all([
               ...
               if(results[0].statusCode !== 200) {
                 ...
                 user.hasPassword = req.body.hasPassword;
                 await user.save();
  ...
  */
  ...
  router.post("/password/recovery", async (req,res) => {
    ...
    try {
      ...
      const account = await models.account.findOne({ where: { id: userAccess.accountId, reqEmail: user.email} });
      if(!account) {
      ...
      if(account.blocked) {
        res.status(400).json({ errors: { backed: ["Account is blocked"]} });
        return;
      }
  ...
  /**
   * @api {post} [dev-]abckend.wazzup24.com/api/v1/users/signup Signup user
...
router.post('/signup', async (req,res) => {
  try {
    ...
    let secondResult = await Promise.all([
    ...
    const emailHtml = mustache.render(template.emailHtml, {
      confirmUrl: http[process.env.NODE_ENV].backendURL + '/api/v1/emailTokens/' + secondResult[1].token + '/handle'
    })
